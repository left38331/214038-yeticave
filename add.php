...
//							' был успешно загружен'; 
	...
	if (!count($errors)) {
		...
		$add_lot = renderTemplate('templates/lot.php', ['new_lots' => $new_lots, 'url_adress' => $url_adress]);
		$condition = 1;
		// подключаем к лойауту и выводим этот шаблон вместо формы
		$layout_content = renderTemplate('templates/layout.php',['title' => 'Новый лот', 'is_auth' => $is_auth, 'user_name' => $user_name, 'user_avatar' => $user_avatar, 'link' => 'href="index.php"', 'add_lot' => $add_lot]);
...
$layout_content = renderTemplate('templates/layout.php',['title' => 'Добавить лот', 'is_auth' => $is_auth, 'user_name' => $user_name, 'user_avatar' => $user_avatar, 'link' => 'href="index.php"', 'add_form' => $add_form]);
print($layout_content);
?>
